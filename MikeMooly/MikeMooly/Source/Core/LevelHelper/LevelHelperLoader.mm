...
//#else
    ...
    //개발 버전이 아닐경우 document에서 얻어옴
    //NSArray* paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory , NSUserDomainMask, YES);
	//NSString* documentsDir = [paths objectAtIndex:0];
    //NSString* pFile = [documentsDir stringByAppendingPathComponent:@"map"];
    //pFile = [pFile stringByAppendingPathComponent:levelFile];
    //pFile = [pFile stringByAppendingPathExtension:@"plhs"];

    //CCLOG(@"FILE = %@",pFile);
	//NSDictionary *dictionary = [NSDictionary dictionaryWithContentsOfFile:pFile];	
